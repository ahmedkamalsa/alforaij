...
def normalize_phone(raw: str) -> tuple[str, str]:
    ...
    # أطوال digits المتوقعة (بما فيها رمز الدولة): (مثالي, تفاوت)
    known_map = {
        "965": ("KW", (11,)), "966": ("SA", (12, 13)), "971": ("AE", (11, 12)),
        "973": ("BH", (11,)), "968": ("OM", (11,)), "974": ("QA", (11,)),
        "20": ("EG", (12,)), "962": ("JO", (12,)), "964": ("IQ", (12,)),
        "961": ("LB", (11,)), "90": ("TR", (12,)), "972": ("IL", (12,)),
        "91": ("IN", (12,)), "86": ("CN", (12, 13)), "81": ("JP", (11, 12)),
        "82": ("KR", (12, 13)), "44": ("GB", (12, 13)), "1": ("US", (11,)),
        "33": ("FR", (11, 12)), "49": ("DE", (12, 13)), "39": ("IT", (11, 12)),
        "34": ("ES", (11, 12)), "7": ("RU", (11,)), "55": ("BR", (12, 13)),
        "52": ("MX", (12, 13)), "61": ("AU", (11, 12)), "967": ("YE", (12,)),
        "963": ("SY", (12,)), "960": ("MV", (11,)), "94": ("LK", (11, 12)),
        "92": ("PK", (12,)), "880": ("BD", (12, 13)), "63": ("PH", (11, 12)),
        "66": ("TH", (10, 11)), "62": ("ID", (11, 12)), "60": ("MY", (10, 11)),
        "65": ("SG", (10, 11)), "84": ("VN", (10, 11)), "98": ("IR", (11, 12)),
        "93": ("AF", (10, 11)), "212": ("MA", (10, 11)), "213": ("DZ", (10, 11)),
        "216": ("TN", (9, 10)), "218": ("LY", (10, 11)), "249": ("SD", (10, 11)),
        "254": ("KE", (10, 11)), "256": ("UG", (10, 11)), "234": ("NG", (11, 12)),
        "27": ("ZA", (10, 11)),
    }

    for code, (country, expected_len) in known_map.items():
        if digits.startswith(code):
            exp = expected_len if isinstance(expected_len, tuple) else (expected_len,)
            if len(digits) in exp:
                return f"+{digits}", country
            # حذف صفر زائد بعد رمز الدولة (مثال: +2001064955051 → +201064955051)
            remaining = digits[len(code):]
            if remaining.startswith("0") and len(remaining) > 1:
                fixed = code + remaining.lstrip("0")
                if len(fixed) in exp:
                    return f"+{fixed}", country
