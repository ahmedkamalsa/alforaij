    ...
    def do_GET(self) -> None:
        ...
        if path == "/api/smart-alerts/list":
            ...
            if supabase_is_configured():
                try:
                    from backend.services.supabase_store import _fetch_rows, SUPABASE_URL
                    rows = _fetch_rows(f"{SUPABASE_URL}/rest/v1/saved_searches?select=name,areas,alert_enabled&user_secret=eq.{secret}&alert_enabled=eq.true") or []
                    json_response(self, {"alerts": rows, "total": len(rows)})
                except Exception:
                    json_response(self, {"alerts": [], "total": 0})
            else:
                json_response(self, {"alerts": [], "total": 0})
